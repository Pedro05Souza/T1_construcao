...
from t1_construcao.shared.auth import get_admin_user, get_current_user_payload
...
def test_get_own_user_data_success(test_client, mocker):
    ...
    mocker.patch(
        ...
        return_value={"id": "user-uuid-67890", "name": "normal-user", "role": "client"},
    )

    response = test_client.get(
        "/api/v1/users/user-uuid-67890",
        headers={"Authorization": "Bearer fake-user-token"},
    )
...
def test_get_other_user_data_as_normal_user_fails(test_client):
    ...
    response = test_client.get(
        "/api/v1/users/another-persons-id",
        headers={"Authorization": "Bearer fake-user-token"},
    )
    ...
    assert (
        ...
        == "Acesso permitido apenas para admin ou o próprio user"
    )
